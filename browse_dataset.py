...
mri_scans: dict[int, list[str]] = {}
for image in image_files:
    # volume_12_slice_140_jpg.rf.b5c7da2fdff2670a257101316c34d6e8.jpg
    volume_num = int(image.split("volume_")[1].split("_slice")[0])
    mri_scans.setdefault(volume_num, []).append(os.path.join(image_dir, image))

print(mri_scans[1])


for k, v in mri_scans.items():
    slices = mri_scans[k]
    imsize = Image.open(slices[0]).size
    print(len(slices), imsize)

# plt.imshow(Image.open(mri_scans[1][0]))
# fig = plt.figure()
# print(len(mri_scans[1]))
# for i, v in enumerate(mri_scans[1][:12]):
#     y = fig.add_subplot(3, 4, i + 1)
#     y.imshow(Image.open(v))
